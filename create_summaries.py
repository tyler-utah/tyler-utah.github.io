...
- **Strong performance**: Up to 3.16× speedup and 1.33× average improvement over OoO CPU across nine diverse benchmarks, with only 1% area and power overhead.
...
- **Fine-grained exploration**: Enables characterizing transformations and errors, studying how performance evolves along optimization sequences.
...
- **Strong speedups**: Geomean 2.14× and maximum 7.59× speedup over homogeneous GPU baselines.
- **Extensibility**: Preliminary exploration with Google's EdgeTPU demonstrates incorporating additional processing unit types.
...
- **Practical utility**: The simplified design enabled rapid autotuning across multiple architectures, finding a geomean speedup of 1.15× and up to 3.32× by specializing parameters to inputs and architectures.
...
- **Massive scale**: Collected data from 106 devices spanning seven vendors — at least 10× larger than prior studies.
- **Bug discovery**: Conformance tests identified two new bugs on embedded Arm and NVIDIA devices.
- **Behavioral insights**: AMD GPUs show 25.3× more weak behaviors on average than Intel; devices can be clustered according to stress testing sensitivity.
...
- **Significant speedups**: Heterogeneous implementations provide up to 13.53× speedup (geomean 3.01×) over homogeneous implementations.
- **Portability insights**: Optimal parameters differ widely across platforms and applications, highlighting the need for flexible decomposition.
...
- **Practical utility**: Demonstrated by reasoning about a producer/consumer buffer, showing that insufficient synchronization improves performance at the cost of occasional data corruption.
...
- **Strong performance**: 2.87× speedup and 8.61× energy efficiency gain over OoO cores in equal-area comparisons; 3× speedup over 64 parallel cores.
...
- **Data peeking**: Enables lossless speedups of more than 5× in tuning stressing parameters.
...
- **Mutation testing evaluation**: Methods validated using mutation testing across multiple GPUs from different vendors.

## Summary

Memory consistency testing relies on litmus tests run many times to detect exceedingly rare violations. The effectiveness depends heavily on stressing parameters (how the test environment pressures the system), yet prior work lacked rigorous methodology for tuning these parameters. This paper provides practical foundations — including efficient tuning, portable parameter selection, and confidence metrics — evaluated across 3 GPUs from 3 vendors.""",
    },
    "rtas2020": {
...
- **Reliable measurement methodology**: A strategy based on percentiles and confidence intervals that provides competitive and reproducible observations, addressing the difficulty of reproducing prior work's interference magnitudes.
...
- **Practical evaluation**: Enemy programs evaluated on AutoBench and CoreMark benchmark suites, achieving statistically larger slowdown than prior work in 35/105 benchmark/chip combinations (max difference 3.8×).
...
- **Case studies**: Demonstrated design space explorations for data science application acceleration and heterogeneous parallel architectures.
...
- **Large empirical study**: 17 graph applications × 3 graph inputs × 6 GPUs spanning multiple vendors, using a graph algorithm DSL compiler targeting OpenCL.
...
- **Key finding**: Existing automatic approaches for building portable optimization policies provide trivial or biased results.
- **Practical insights**: Fully portable approach provides 1.15× improvement; semi-specialization to application and input (not hardware) achieves 1.29× — revealing performance-critical features of specialization.

## Summary

Optimizing graph applications for GPUs is labor-intensive due to complex interactions between GPU architectures, applications, and inputs. This paper provides a rigorous methodology to quantify exactly how much performance is sacrificed for portability, and shows that semi-specialization (optimizing for some dimensions while remaining portable across others) offers a practical middle ground.""",
    },
    "concur2018": {
...
- **Two new scheduler models**: Designed to support existing GPU applications; analysis shows one enables more natural implementation of GPU protocols.
- **Blocking idiom analysis**: Examines barrier, mutex, and producer-consumer idioms under each scheduler model.

## Summary

GPU programming models like OpenCL provide almost no fairness guarantees, yet many useful applications rely on blocking synchronization (mutexes, barriers) that require some degree of fairness. This paper bridges the gap by formalizing semi-fair schedulers — a middle ground — and proposing new scheduler models that capture the actual behavior of deployed GPUs more accurately than existing specifications.""",
    },
    "pldi2018": {
...
- **Key finding**: A proposed TM extension to ARMv8 (under consideration within ARM Research) is incompatible with lock elision without sacrificing portability or performance.
- **Counter-example**: Automatically contradicts the claim from Rajwar and Goodman's seminal lock elision paper that "correctness is guaranteed without any dependence on memory ordering."

## Summary

Both weak memory models and transactional memory are well-studied individually, but their interaction is poorly understood — problematic because x86, Power, and C++ all support both. This paper provides the first formal treatment of their combined semantics, backed by automated tools that generated test cases and found a significant incompatibility in an ARM research proposal.""",
    },
    "fse2017": {
...
- **Cooperative kernel model**: Workgroups are fairly scheduled and can cooperate with the scheduler through language extensions, enabling blocking algorithms (mutexes, barriers) on GPUs.
- **GPU multitasking**: Kernels can flexibly resize to share the GPU with other workloads (e.g., graphics rendering), something not supported by prior approaches that exploit scheduling quirks.
- **OpenCL 2.0 prototype**: Implemented and evaluated with blocking GPU applications ported to cooperative kernels.

## Summary

Irregular data-parallel algorithms increasingly target GPUs but require blocking synchronization (barriers, mutexes) that demands fair scheduling. GPU programming models don't mandate fair scheduling, and current approaches exploit undocumented scheduling behavior. Cooperative kernels provide a principled solution through a small set of language extensions enabling kernel-scheduler cooperation, supporting both fair scheduling and GPU multitasking.""",
    },
    "popl2017": {
...
- **Key insight**: While constraints over programs directly are intractable, analogous constraints over program *executions* can be solved, then programs constructed from solutions.
...
- **Practical impact**: Prior approaches either required splitting kernels (multi-kernel method) or made unsafe assumptions about GPU occupancy.

## Summary

Inter-workgroup barriers are essential for many GPU applications but not supported by GPU programming models because the occupancy-bound execution model can cause deadlock. This paper solves the problem by dynamically discovering how many workgroups can safely execute concurrently, then restricting the barrier accordingly. The approach is formally verified, portable across vendors, and evaluated on practical applications.""",
    },
    "pldi2016": {
...
- **Novel testing environment**: Sophisticated memory stressing strategy tuned per chip using results from nearly half a billion micro-benchmark executions — no prior knowledge of the application required.
- **Practical bug discovery**: Evaluated on 7 GPUs × 3 Nvidia architectures × 10 CUDA applications; discovered previously unknown weak memory issues in two applications.
- **Automatic fence suggestion**: Testing environment identifies root causes and suggests fence insertions to harden applications against weak memory bugs.
- **Fence overhead analysis**: Benchmarks the runtime and energy cost of fence insertion strategies.

## Summary

GPU applications can exhibit subtle bugs due to weak memory effects — where memory operations execute out of order. These bugs are extremely difficult to reproduce normally. This paper provides the first practical method for systematically provoking and detecting such bugs in real GPU applications, using a carefully tuned stressing environment that makes rare weak memory behaviors occur frequently.""",
    },
    "asplos2015": {
...
- **False beliefs exposed**: Folklore assumptions about GPU memory ordering — often supported by official tutorials — proven false by observing previously elusive weak behaviors.
- **Formal GPU memory model**: A variant of SPARC Relaxed Memory Order (RMO), structured to follow the GPU concurrency hierarchy, correctly modeling every observed behavior.

## Summary

GPU concurrency was poorly specified, forcing programmers to rely on unstated assumptions. This foundational paper conducted the first systematic empirical study of GPU memory behavior, developing tools to generate and execute thousands of concurrent tests. The results exposed many false assumptions in vendor documentation and provided the first formal memory model for Nvidia GPUs.""",
    },
    "iccad2020": {
...
- **DECADES project**: Multi-team effort to design and tape out a new heterogeneous architecture; the tools enable rapid evaluation of design decisions across the entire stack.
...
With the end of transistor scaling trends, performance improvements now come from specialization — designing heterogeneous systems of specialized processing units. This paper presents the DEC++ compiler and MosaicSim simulator as complementary tools for exploring the vast design space of heterogeneous architectures, enabling rapid evaluation of choices across programming models, compilers, ISAs, and specialized hardware.""",
    },
    "iwocl2019": {
        "title": "Performance Evaluation of OpenCL Standard Support (and Beyond)",
        "authors": "T. Sorensen, S. Pai, A. F. Donaldson",
        "venue": "IWOCL, 2019 (Best Paper Award)",
        "summary": """This paper evaluates how support for various OpenCL features across GPU vendors affects the performance of graph applications, using a DSL compiler with six independent optimizations.

## Key Contributions

- **Controlled study**: 6 optimizations × 17 applications × 3 graph inputs × 6 GPUs from 4 vendors (Nvidia, AMD, Intel, ARM), with each optimization requiring a specific minimum OpenCL version.
- **Key finding**: Limiting to OpenCL 2.0 features fails to achieve speedups in 70%+ of benchmarks; OpenCL 2.1 features reduce this to 60%; forward progress guarantees (beyond the standard) unlock the best optimizations.
...
OpenCL promises platform portability, but adoption of OpenCL features varies widely across vendors. This paper quantifies the performance impact of progressively more advanced OpenCL features (from 1.x through 2.1 and beyond) on graph applications, providing concrete motivation for vendors to adopt newer features and for the standard to include forward progress guarantees.""",
    },
    "iwocl2016": {
...
- **Community analysis**: Examination of 50 recent GPU papers showing 58% evaluate GPUs from a single vendor only, indicating the portability challenge is widespread.
- **Practical guidance**: Lessons learned and recommendations for improving cross-platform OpenCL development.

## Summary

While OpenCL promises cross-platform portability, in practice many applications fail when run on platforms from different vendors. This paper systematically documents the portability issues encountered — from vendor framework bugs to ambiguous specifications to programming errors — and provides practical insights for developers seeking genuinely portable GPU code.""",
    },
    "tinytocs2015": {
...
- **Demonstration by example**: The paper's own text is passed through a GPU cipher program using a published (buggy) mutex from the book *CUDA by Example*, producing visibly corrupted output — then corrected with proper synchronization.
- **Methodology**: Systematic testing of deployed GPUs (and CPUs) to discover and correct false memory ordering assumptions.

## Summary

A creative and memorable demonstration that common GPU programming examples contain memory ordering bugs. By literally passing the paper's text through a broken cipher program, the authors make the abstract concept of weak memory bugs tangible, while advocating for rigorous testing over trusting published folklore.""",
    },
    "ics2013": {
...
- **Hardware testing**: Extended litmus testing tools to GPUs, revealing that older Nvidia Kepler chips exhibit controversial relaxed coherence behavior while newer Maxwell chips do not.
- **Bug discovery**: Showed a published GPU spin-lock implementation from *CUDA by Example* provides insufficient synchronization.

## Summary

This foundational undergraduate work established the first systematic approach to studying GPU memory consistency models. It introduced GPU-specific litmus tests, proposed a formal operational model, and demonstrated through hardware testing that GPU memory behavior differs across architectures — setting the stage for an extensive line of research on GPU memory models.""",
    },
}

# Create individual summary files
for paper_id, info in papers.items():
    filepath = os.path.join(summaries_dir, f"{paper_id}.md")
    content = f"""# {info['title']}

**Authors:** {info['authors']}
**Venue:** {info['venue']}
**PDF:** [files/{paper_id}.pdf](../{paper_id}.pdf)
...
# Create combined summaries file
combined_path = os.path.join(os.path.dirname(__file__), "files", "summaries.md")
with open(combined_path, "w", encoding="utf-8") as f:
    f.write("# Paper Summaries\n\n")
    f.write("Summaries of research papers by Tyler Sorensen and collaborators.\n\n")
    f.write("---\n\n")

    # Group by approximate year/section
    sections = [
        ("2026", ["disorder", "parallelx2025"]),
        ("2025", ["saferace", "shadow", "peak2025", "bettertogether2025", "miniGiraffe"]),
        ("2024", ["ghost", "mix_testing", "leftoverlocals2024"]),
        ("2023", ["gpuharbor2023", "mcmutants2023", "redwood2023"]),
        ("2021", ["oopsla2021b", "oopsla2021a", "taco2021"]),
        ("2020", ["oopsla2020", "rtas2020", "ispass2020"]),
        ("2019", ["iiswc2019"]),
        ("2018", ["concur2018", "pldi2018"]),
        ("2017", ["fse2017", "popl2017"]),
        ("2016", ["oopsla2016", "pldi2016"]),
        ("2015", ["asplos2015"]),
        ("Workshop Papers", ["iccad2020", "iwocl2019", "iwocl2016", "tinytocs2015", "ics2013"]),
    ]

    for section_name, paper_ids in sections:
        ...
        for pid in paper_ids:
            ...
            f.write(f"**PDF:** [{pid}.pdf](../{pid}.pdf) | **Full Summary:** [summaries/{pid}.md](summaries/{pid}.md)\n\n")
            f.write(f"{info['summary']}\n\n")
            f.write("---\n\n")

print(f"Created {len(papers)} individual summary files in {summaries_dir}")
print(f"Created combined summary at {combined_path}")
